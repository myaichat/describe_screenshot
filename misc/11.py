...
class CoordinatesFrame(wx.Frame):
    ...
    def take_screenshot(self, coordinates):
        ...
        # Resize the screenshot to fit the canvas size while maintaining aspect ratio
        image_bytes = io.BytesIO()
        screenshot.save(image_bytes, format="PNG")
        image_bytes.seek(0)
        pil_image = Image.open(image_bytes)

        pil_image.thumbnail(self.canvas_size)  # Resize to fit canvas
        wx_image = wx.Image(*pil_image.size)
        wx_image.SetData(pil_image.convert("RGB").tobytes())
        self.screenshot_bitmap = wx_image.ConvertToBitmap()

        # Display the resized screenshot on the canvas
